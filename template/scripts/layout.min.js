...
ff = ff.replace(XRegExp("[\\p{Mn}\\u0640\\u200C]", 'gmi'),'').replace(XRegExp("[\\p{S}\\p{P}\\u06E9\\u06DE{}\"'()\\uFD3F\\uFD3E]", 'gmi'), ' ').replace(/\s\s+/gmi, ' ').replace(XRegExp("^\\s+|\\s+$", 'gmi'),"").replace(XRegExp("[أإآ\u0671]", 'gmi'),"ا").replace(XRegExp("\u0629", 'gmi'),"\u0647").replace(XRegExp("\u0624", 'gmi'),"\u0648").replace(XRegExp("\u0626", 'gmi'),"\u0649").replace(/[\u0660\u0661\u0662\u0663\u0664\u0665\u0666\u0667\u0668\u0669]/g, function (m) {
        return {
        '\u0660': '0',
        '\u0661': '1',
		'\u0662': '2',
...
/*return a=d.get(c("KEY_CAN_HANDLE_SEARCH")),f=hh||null,e={},e[c("HASH_KEY_RH_SEARCH")]=f,e[c("HASH_KEY_UIMODE")]="search",a!==!0?document.location=window.gRootRelPath+"/"+(window.gSearchPageFilePath||c("START_FILEPATH"))+("?#"+b.mapToEncodedString(e)):(d.publish(c("EVT_QUERY_SEARCH_RESULTS"),!0),b.defer(function(){return b.updateHashMap(e,!0)}));*/

}),d.subscribe(c("EVT_PROJECT_LOADED"),function(){return window.readSetting&&window.readSetting(RHANDSEARCH,function(a){return d.publish(c("KEY_AND_SEARCH"),a)}),d.subscribe(c("KEY_AND_SEARCH"),function(a){return saveSetting(RHANDSEARCH,"1"===a?"1":"0",!0)})}),d.subscribe(c("EVT_QUERY_SEARCH_RESULTS"),function(){
...
var d;if(13===b&&(d=this.widget.get(p("KEY_SEARCH_TERM")),d!==a&&this.widget.publish(p("KEY_SEARCH_TERM"),a),this.widget.publish(p("EVT_SEARCH_TERM"),!0),null!=c?c.target:void 0))return c.target.blur()
